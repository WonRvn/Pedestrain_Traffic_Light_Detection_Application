...
class MainActivity : AppCompatActivity(){
    ...
    private lateinit var startScreenView: TextureView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        startScreenView = findViewById(R.id.camera_preview)
        ttsManager = TTSManager(this)

//        ttsManager.speak("화면을 두 번 터치하면 신호등 탐지가 시작됩니다!")

        val doubleTapListener = DoubleTapListener(this) {
            setContentView(R.layout.main_camera)
            previewView = findViewById(R.id.previewView)
            rectView = findViewById(R.id.rectView)
            imageView = findViewById(R.id.imageView) // Make sure this ID exists in your layout and is correctly assigned

            // Initialize dataProcess here, after imageView has been initialized
            dataProcess = DataProcess(context = this, ttsManager, imageView)
        ...
        startScreenView.setOnTouchListener { _, event ->
    ...
    override fun onDestroy() {
        ...
        // Clean up the ONNX session when the activity is destroyed to free up resources
        session.close()
        ortEnvironment.close() // If applicable, close the OrtEnvironment as well
